...
export default {
    async single(req: IReqUser, res: Response) {
        if (!req.file) response.error(res, null, "File not found");
        try {
    ...
    async multiple(req: IReqUser, res: Response) {
        if (!req.files || req.files.length === 0) response.error(res, null, "File not found");
        try {
    ...
    async remove(req: IReqUser, res: Response) {
        try {
            ...
            if (!fileUrl) response.error(res, null, "File URL not found");
